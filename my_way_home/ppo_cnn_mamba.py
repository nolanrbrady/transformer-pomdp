...
class PPOAgent(nn.Module):
    def __init__(self, cnn_encoder, action_dim, hidden_dim=512, context_length=128):
        ...
        self.mamba = MambaModel(self.mamba_config).to(self.device)
    ...
    def forward(self, obs):
        ...
        if obs.dim() == 4:
            ...
            for i in range(batch_size):
                ...
                mamba_output = self.mamba(inputs_embeds=feature_seq)
                mamba_hidden_state = mamba_output.last_hidden_state
                cls_token = mamba_hidden_state[:, 0, :]
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PPO Mamba")

...
# === RUDDER LSTM Reward Redistribution ===
class RudderLSTM(nn.Module):
    ...
    def forward(self, feature_seq):
        # feature_seq: [B, T, D] or [T, D]
        if feature_seq.dim() == 2:
            feature_seq = feature_seq.unsqueeze(0)  # [1, T, D]
        lstm_out, _ = self.lstm(feature_seq)
        reward_pred = self.fc(lstm_out).squeeze(-1)  # [B, T]
        return reward_pred
...
def train(env_id="VizdoomMyWayHome-v0", total_timesteps=500_000, rollout_len=4096, batch_size=64, 
    ...
    rudder = RudderLSTM(input_dim=512, hidden_dim=256).to(device)
    print("Models initialized on device")

    # Optimizer
    optimizer = torch.optim.Adam(list(agent.parameters()) + list(rudder.parameters()), lr=lr)
    ...
    for iteration in range(0, total_timesteps, rollout_len):
        ...
        for t in range(rollout_len):
            ...
            feature_seq.append(feature.squeeze(0).cpu())
        ...
        # Prepare feature sequence for RUDDER: [T, D]
        feature_seq_tensor = torch.stack(feature_seq, dim=0).to(device)  # [T, D]
        # Compute redistributed rewards
        rudder.train()
        redistributed_rewards = rudder(feature_seq_tensor).detach().cpu().squeeze().tolist()
        # Replace rewards in buffer with redistributed rewards
        ...
        for epoch in range(K_epochs):
            for obs_batch, act_batch, logp_batch, adv_batch, ret_batch, feat_batch in buffer.get_batches(batch_size):
                ...
                # === RUDDER loss ===
                # Predict return from feature sequence
                rudder.train()
                # feat_batch: [B, D], need [B, T, D] for LSTM, so use as is for simplicity
                # For joint training, use MSE between sum of predicted rewards and actual returns
                pred_rewards = rudder(feat_batch.unsqueeze(1)).sum(dim=1)  # [B]
                rudder_loss = F.mse_loss(pred_rewards, ret_batch)
                # === End RUDDER ===
                
                # Total loss
                total_loss = policy_loss + vf_coef * value_loss - ent_coef * entropy_loss + rudder_loss
                ...
                rudder_losses.append(rudder_loss.item())
        ...
        print(f"|    rudder_loss          | {np.mean(rudder_losses):<5.3f} |")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rollout_len", type=int, default=8400)

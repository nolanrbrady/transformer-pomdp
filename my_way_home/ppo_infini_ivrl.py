...
class NeedRewardModule(nn.Module):
    def __init__(self, input_dim, hidden_dim=512, need_names=None):
        super().__init__()
        self.need_names = need_names or ["novelty", "uncertainty", "controllability", "saliency", "goal_progress"]
        self.num_needs = len(self.need_names)
        self.need_weights_mlp = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, self.num_needs)
        )
        self.static_weights = nn.Parameter(torch.ones(self.num_needs))  # optional: use either static or dynamic

    def forward(self, feat, need_values_dict):
        """
        feat: ViT features [B, F]
        need_values_dict: dict of {name: value} for each need (scalar tensors)
        Returns: total_reward [B], individual need rewards [B, N]
        """
        # Compute dynamic weights based on context
        logits = self.need_weights_mlp(feat)
        weights = F.softmax(logits, dim=-1)  # [B, N]

        # Stack need values in consistent order
        values = torch.stack([need_values_dict[name] for name in self.need_names], dim=1)  # [B, N]

        # Weighted sum
        total_reward = (weights * values).sum(dim=1)  # [B]
        return total_reward, weights

class LatentNeedBank(nn.Module):
    """
    Estimates intrinsic needs (unsupervised) from ViT features:
    - Novelty
    - Uncertainty
    - Controllability
    - Temporal Saliency
    ...
    def __init__(self, input_dim=512, hidden_dim=128):
        super().__init__()
        self.need_names = ["novelty", "uncertainty", "controllability", "saliency", "goal_progress"]
        ...
        self.controllability_mlp = nn.Sequential(
            nn.Linear(input_dim+1, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, input_dim)
        )

        # Memory for temporal and goal tracking
        self.prev_feature = None
        self.prev_goal_score = None

    def forward(self, feature, action=None):
        """
        Input:
            feature: [B, D] ViT feature embedding
        Returns:
            dict of {need_name: [B] tensor}
        """
        values = {}

        for name in self.need_names:
            values[name] = self.heads[name](feature).squeeze(-1)  # shape: [B]

        # Uncertainty = std of samples
        if self.training:
            samples = torch.stack([
                self.heads['uncertainty'](F.dropout(feature, p=0.2, training=True)).squeeze(-1)
                for _ in range(10)
            ])
            values['uncertainty'] = samples.std(dim=0)
        else:
            values['uncertainty'] = self.heads['uncertainty'](feature).squeeze(-1)

        # Temporal Saliency = change in feature space
        if self.prev_feature is not None:
            saliency = (feature - self.prev_feature).pow(2).mean(dim=1)
            values["saliency"] = saliency

        # Controllability = predict next feature
        if self.prev_feature is not None and action is not None:
            # Ensure the action can be used with batches and without.
            if isinstance(action, (int, float)):
                action_tensor = torch.full((feature.size(0), 1), float(action), device=feature.device)
            elif isinstance(action, torch.Tensor):
                if action.dim() == 0:
                    action_tensor = action.view(1, 1).float().to(feature.device)
                elif action.dim() == 1:
                    action_tensor = action.unsqueeze(1).float().to(feature.device)
                else:
                    action_tensor = action.float().to(feature.device)
            
            control_input = torch.cat([self.prev_feature, action_tensor], dim=1)
            pred_next = self.controllability_mlp(control_input)
            controllability_error = F.mse_loss(pred_next, feature, reduction="none").mean(dim=1)
            values["controllability"] = F.relu(1.0 - controllability_error)

        # Goal Progress = improvement in goal score
        if self.prev_goal_score is not None:
            goal_score = values["goal_progress"]
            progress = (goal_score - self.prev_goal_score).clamp(min=0)
            values["goal_progress"] = progress
            self.prev_goal_score = goal_score.detach()
        else:
            self.prev_goal_score = values["goal_progress"].detach()
            values["goal_progress"] = torch.zeros_like(values["goal_progress"])
        
        self.prev_feature = feature.detach()
...
def train(env_id="VizdoomMyWayHome-v0", total_timesteps=500_000, rollout_len=4096, batch_size=64, K_epochs=10,
    ...
    model_path = os.path.join(save_dir, f"{env_id.split('-')[0]}_ppo_ivrl_vit_best.pt")
    ...
    need_reward_module = NeedRewardModule(input_dim=512).to(device)
    latent_need_bank = LatentNeedBank(input_dim=512).to(device)
    ...
    latent_need_bank_optimizer = torch.optim.Adam(latent_need_bank.parameters(), lr=1e-4)
    need_reward_module_optimizer = torch.optim.Adam(need_reward_module.parameters(), lr=1e-4)
    ...
    for iteration in range(0, total_timesteps, rollout_len):
        ...
        for t in range(rollout_len):
            ...
            latent_need_bank_values = latent_need_bank(feature, action)
            intrinsic_reward, weights = need_reward_module(feature, latent_need_bank_values)
            intrinsic_reward_scale = 1.0
            total_reward = extrinsic_reward + intrinsic_reward * intrinsic_reward_scale
            episode_reward += total_reward.item() if isinstance(total_reward, torch.Tensor) else total_reward
            episode_length += 1

            # Monitor rewards over time    
            if t % 1000 == 0:
                print(f"Extrinsic: {extrinsic_reward:.2f}, Intrinsic: {intrinsic_reward.item():.2f}, Total: {total_reward.item():.2f}")
                print("Need weights:", {k: round(w, 2) for k, w in zip(latent_need_bank.need_names, weights[0].cpu().tolist())})
            ...
            if done:
                # Append the reward as a scalar
                episode_rewards.append(episode_reward if not isinstance(episode_reward, torch.Tensor) else episode_reward.item())
        ...
        for epoch in range(K_epochs):
            ...
            y_pred = np.array([v.cpu().item() if isinstance(v, torch.Tensor) else v for v in buffer.values])
            y_true = np.array([r.cpu().item() if isinstance(r, torch.Tensor) else r for r in buffer.returns])
            y_var = np.var(y_true)

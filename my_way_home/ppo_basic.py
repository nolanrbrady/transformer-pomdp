#!/usr/bin/env python
# Custom PPO implementation with BasicViT for ViZDoom My Way Home
import os
import sys
import time
import numpy as np
...
class ViTFeatureWrapper(nn.Module):
    ...
    def forward(self, obs):
        ...
        # BasicViT doesn't need a temporal sequence, so we directly process
        # the observation (single or batch)
        return self.vit(obs_tensor)
...
# === Training Loop ===
def train(env_id="VizdoomMyWayHome-v0", total_timesteps=1_000_000, rollout_len=4096, batch_size=64, 
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PPO BasicViT")

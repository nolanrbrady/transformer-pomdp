#!/usr/bin/env python
# Custom PPO implementation with CNN-LSTM for ViZDoom My Way Home
import os
import sys
import time
import numpy as np
...
class PPOAgent(nn.Module):
    def __init__(self, cnn_encoder, action_dim, hidden_dim=512, context_length=128):
        super().__init__()
        ...
        # Temporal modeling with LSTM (3 stacked layers)
        self.lstm = nn.LSTM(self.flattened_dim, hidden_dim, num_layers=3, batch_first=True).to(self.device)

        # Policy and value heads use LSTM hidden dimension
        self.policy = nn.Linear(hidden_dim, action_dim).to(self.device)
    ...
    def forward(self, obs):
        ...
        if obs.dim() == 4:
            ...
            for i in range(batch_size):
                ...
                # Run sequence through LSTM and take last timestep
                lstm_out, _ = self.lstm(feature_seq)
                cls_token = lstm_out[:, -1, :]
                
                logits = self.policy(cls_token)
                value = self.value(cls_token).squeeze(-1)
                outputs.append((logits, value, feat))
            # Stack outputs
...
def train(env_id="VizdoomMyWayHome-v0", total_timesteps=500_000, rollout_len=4096, batch_size=64, 
    ...
    model_path = os.path.join(save_dir, f"{env_id.split('-')[0]}_ppo_cnn_mamba_best.pt")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PPO CNN-LSTM")
